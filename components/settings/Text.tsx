...
const RenderSelect = ({
  ...
  >
    <SelectTrigger className='no-ring w-full rounded-sm border border-gray-200'>
      <SelectValue
        placeholder={
          config.property === "fontFamily"
            ? "Choose a font"
    ...
    <SelectContent className='border-primary-grey-200 bg-black text-gray-300'>
      {config.options.map((option) => (
        <SelectItem
          key={option.value}
          value={option.value}
          className=' hover:bg-green-500 hover:text-primary-black'
        >
          {option.label}
        </SelectItem>
      ))}
